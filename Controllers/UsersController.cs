...
{
    [HttpGet]
    public async Task<IReadOnlyList<UserView>> List([FromQuery] string? role, [FromQuery] string? companyId, [FromQuery] bool includeInactive = false)
    {
        var query = db.Users.AsNoTracking().AsQueryable();
        if (!includeInactive) query = query.Where(x => x.Active);
        if (!string.IsNullOrWhiteSpace(role)) query = query.Where(x => x.Role == role);
        if (!string.IsNullOrWhiteSpace(companyId)) query = query.Where(x => x.CompanyId == companyId);
    ...
    public async Task<IActionResult> Create(CreateUserRequest request)
    {
        ...
        var user = new User { Name = request.Name.Trim(), Username = username, PasswordHash = "", Role = NormalizeRole(request.Role), CompanyId = Clean(request.CompanyId), Territory = request.Territory.Trim(), Active = request.Active };
    ...
    public async Task<IActionResult> Update(Guid id, UpdateUserRequest request)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.Name) || role is not ("Admin" or "Rep") || role == "Rep" && string.IsNullOrWhiteSpace(request.CompanyId))
        ...
        user.Name = request.Name.Trim(); user.Role = role; user.CompanyId = role == "Admin" ? null : Clean(request.CompanyId);
    ...
    private static string? Validate(string name, string username, string password, string role, string? companyId)
    {
        ...
        if (normalized == "Rep" && string.IsNullOrWhiteSpace(companyId)) return "A company is required for a sales rep.";
        return null;
    }
